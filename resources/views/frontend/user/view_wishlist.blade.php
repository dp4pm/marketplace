...
@section('panel_content')
    <div class="plx-titlebar mt-2 mb-lg-3 mb-md-2 mb-sm-1 mb-1">
        <div class="row align-items-center">
            <div class="col-md-6">
                <div class="section-title-main fw-600">{{ translate('Wishlist')}}</div>
            </div>
        </div>
    </div>
        ...
        <div class="row gutters-5">
            @forelse ($wishlists as $key => $wishlist)
                @if ($wishlist->product != null)
                    <div class="col-xxl-3 col-xl-4 col-lg-4 col-md-4 col-sm-6 custom-col-xs" id="wishlist_{{ $wishlist->id }}">
    ...
    <script type="text/javascript">
        function removeFromWishlist(id){
            $.post('{{ route('wishlists.remove') }}',{_token:'{{ csrf_token() }}', id:id}, function(data){
                $('#wishlist').html(data);
                $('#wishlist_'+id).hide();
                PLX.plugins.notify('success', '{{ translate('Item has been renoved from wishlist') }}');
            })
        }
    </script>
@endsection

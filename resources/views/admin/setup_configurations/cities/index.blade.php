                    ...
                    <div class="card-header row gutters-5">
                        ...
                        <div class="col-md-9 d-flex justify-content-end">
                            ...
                            <div class="form-group mb-0 ml-3">
                                <select class="form-control plx-selectpicker w-100" data-live-search="true" id="sort_state" name="sort_state">
                        ...
                        <tbody>
                            @foreach($cities as $key => $city)
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <a class="btn btn-action-button btn-icon btn-circle btn-sm" href="{{ route('cities.edit', ['id'=>$city->id, 'lang'=>env('DEFAULT_LANGUAGE')]) }}" title="{{ translate('Edit') }}">
                                            <i class="las la-edit"></i>
                                        </a>
                                        <a href="#" class="btn btn-action-button btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('cities.destroy', $city->id)}}" title="{{ translate('Delete') }}">
    			...
    			<div class="card-body">
    				<form action="{{ route('cities.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <select class="select2 form-control plx-selectpicker" name="state_id" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                                @foreach ($states as $state)
    ...
    <script type="text/javascript">
        ...
        function update_status(el){
            ...
            $.post('{{ route('cities.status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    PLX.plugins.notify('success', '{{ translate('Country status updated successfully') }}');
                }
                else{

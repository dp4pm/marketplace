...
<div class="card">
    <form class="" id="sort_blogs" action="" method="GET">
        ...
        <div class="card-body">
            <table class="table mb-0 plx-table">
                <thead>
                    <tr>
                        <th>#</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-right">
                            <a class="btn btn-action-button btn-icon btn-circle btn-sm" href="{{ route('blog.edit',$blog->id)}}" title="{{ translate('Edit') }}">
                                <i class="las la-pen"></i>
                            </a>

                            <a href="#" class="btn btn-action-button btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('blog.destroy', $blog->id)}}" title="{{ translate('Delete') }}">
    ...
    <script type="text/javascript">
        function change_status(el){
            ...
            $.post('{{ route('blog.change-status') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    PLX.plugins.notify('success', '{{ translate('Change blog status successfully') }}');
                }
                else{

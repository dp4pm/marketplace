...
<div class="card">
    <div class="card-header d-block d-md-flex">
        <h5 class="section-title mb-0">{{ translate('All Categories') }}</h5>
            ...
            <tbody>
                @foreach($categories as $key => $category)
                    <tr>
                        ...
                        <td class="text-right">
                            <a class="btn btn-action-button btn-icon btn-circle btn-sm" href="{{route('categories.edit', ['id'=>$category->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
                                <i class="las la-edit"></i>
                            </a>
                            <a href="#" class="btn btn-action-button btn-icon btn-circle btn-sm confirm-delete" data-href="{{route('categories.destroy', $category->id)}}" title="{{ translate('Delete') }}">
    ...
    <script type="text/javascript">
        function update_featured(el){
            ...
            $.post('{{ route('categories.featured') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    PLX.plugins.notify('success', '{{ translate('Featured categories updated successfully') }}');
                }
                else{

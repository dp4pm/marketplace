...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="section-title mb-0">{{translate('Seller Withdraw Request')}}</h5>
        </div>
        <div class="card-body">
                    ...
                    @foreach($seller_withdraw_requests as $key => $seller_withdraw_request)
                        @if (\App\Models\Seller::find($seller_withdraw_request->user_id) != null && \App\Models\Seller::find($seller_withdraw_request->user_id)->user != null)
                            <tr>
                                ...
                                <td class="text-right">
    		                            <a class="btn btn-action-button btn-icon btn-sm"  onclick="show_seller_payment_modal('{{$seller_withdraw_request->user_id}}','{{ $seller_withdraw_request->id }}');" title="{{ translate('Pay Now') }}">
    		                                <i class="las la-money-bill"></i>
    		                            </a>
                                    <a class="btn btn-action-button btn-icon btn-sm"  onclick="show_message_modal('{{ $seller_withdraw_request->id }}');" title="{{ translate('Message View') }}">
    		                                <i class="las la-eye"></i>
    		                            </a>
    		                            <a href="{{route('sellers.payment_history', encrypt($seller_withdraw_request->user_id))}}" class="btn btn-action-button btn-circle btn-sm"  title="{{ translate('Payment History') }}">

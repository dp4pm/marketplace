...
class PlxUploadController extends Controller
{
    ...
    public function show_uploader(Request $request){
        return view('uploader.plx-uploader');
    }
    public function upload(Request $request){
        ...
        if($request->hasFile('plx_file')){
            $upload = new Upload;
            $extension = strtolower($request->file('plx_file')->getClientOriginalExtension());

            if(isset($type[$extension])){
                $upload->file_original_name = null;
                $arr = explode('.', $request->file('plx_file')->getClientOriginalName());
                ...
                $path = $request->file('plx_file')->store('uploads/all', 'local');
                $size = $request->file('plx_file')->getSize();
                ...
                if($type[$extension] == 'image' && get_setting('disable_image_optimization') != 1){
                    try {
                        $img = Image::make($request->file('plx_file')->getRealPath())->encode();
                        $height = $img->height();
                        $width = $img->width();
